...
function getCart() {
    ...
    produitLocalStorage.map(p => {
        fetch(`http://localhost:3000/api/products/${p.productId}`).then((v) => {
            v.json().then(produit => {
                console.log(produit)
                ...
                productColor.innerHTML = p.color;
                ...
                function getTotals() {
                    ...
                    for (let i = 0; i < myLength; ++i) {
                        totalPrice += (elemsQtt[i].valueAsNumber * produit.price);
                    }
